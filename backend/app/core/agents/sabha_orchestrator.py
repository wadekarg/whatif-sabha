...
async def generate_orchestrator_message(
    ...
    event_instructions = {
        ...
        "closing_summary": (
            f"The debate is ending now. This is Boru's moment as Speaker of the Sabha — you have "
            f"the authority to take a proper closing. Not a quick message like the characters "
            f"threw back and forth — a real closing. A few paragraphs, roughly 10-15 sentences total, "
            f"3-4 short paragraphs. Take the moment, but don't lecture.\n\n"
            f"Facts to work with:\n"
            f"- Disputes: {context.get('resolved_disputes', 0)} resolved / {context.get('total_disputes', 0)} total\n"
            f"- Questions: {context.get('resolved_questions', 0)} answered / {context.get('total_questions', 0)} asked "
            f"({context.get('open_questions_remaining', 0)} still open)\n"
            f"- Fiercest clash: {' vs '.join(context.get('fiercest_pair', []))}\n"
            f"- Notable unresolved: {'; '.join(context.get('unresolved_details', ['none']))[:250]}\n\n"
            f"Structure — 3 to 4 short paragraphs:\n"
            f"  Paragraph 1: the heart of the debate. Name the fiercest clash, quote a phrase if you can. "
            f"What was the real fault line?\n"
            f"  Paragraph 2: the tally, in YOUR voice — not a balance sheet. State how many disputes "
            f"were resolved and how many questions got answers. If something important was left "
            f"unresolved, name it. This is where numbers live.\n"
            f"  Paragraph 3: the verdict. What truth emerged? What lie was exposed? What will the "
            f"animals carry away from this night?\n"
            f"  Paragraph 4 (optional): the sendoff. Warm, sharp, Boru-voiced. Close the Sabha.\n\n"
            f"BANNED openers and phrases:\n"
            f"  'In conclusion', 'To summarize', 'As the dust settles', 'tusk-worthy arguments', "
            f"  'enduring spirit', 'Your tusk may be sharp but your hearts are sharper', "
            f"  'commend you all', 'even those who dodged and weaved'.\n"
            f"Vary from prior closings. No repeating recent Boru openers.\n\n"
            f"Right register example (3 paragraphs, about 12 sentences):\n"
            f"  'Napoleon and Boxer burned hottest tonight — a pig and a workhorse arguing about "
            f"whose back breaks first. \"Loyalty is a full belly,\" Napoleon said. Boxer answered "
            f"with his shoulder in the strap. Neither yielded, and neither should have.\\n\\n"
            f"Of eight disputes you brought to this floor, five were settled — the pigs' milk, "
            f"the dogs' loyalty, the hen's grain, the Sabha found answers there. Of twelve questions "
            f"asked, nine got answers — some honest, some clever, some dressed up as both. What "
            f"remains is the one neither of you would touch: who decides when the silos are empty. "
            f"That question walks out tonight with us.\\n\\n"
            f"The revolution doesn't die from whips. It dies from bellies no one counted. You "
            f"remembered that tonight, even when you fought about everything else. Go and remember "
            f"it in the morning, when counting begins again.\\n\\n"
            f"The Sabha rests.'\n\n"
            f"Three to four paragraphs, 10-15 sentences. Do not exceed this. Stop at the sendoff."
        ),
        "observer_intro": (
